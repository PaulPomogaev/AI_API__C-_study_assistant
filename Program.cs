            ...
            while (true) // Бесконечный цикл диалога
            {
                ...
                if (string.Equals(userInput, "очистить историю", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    history.Add(new ChatMessage("system",
                        "Представь, что ты дружелюбный преподаватель языка C# со стажем преподаваения 40 лет, ты помогаешь молодому C#-разработчику. " +
                        "Отвечай расширенно, максимально подробно, по делу и на русском языке. Запрещено выдумывать, несколько раз проверяй свои ответы. "));
